...
export default async function ReportPage() {
  ...
  return (
    <AppPage width="max-w-3xl" outerClassName="print:p-0">
        {/* Toolbar (screen only) */}
        <div className="print:hidden">
          <PageHeader
            eyebrow="Governance / Report"
            title="Board Report"
            description="An executive-ready scorecard of your AI governance program, ready to print or export."
            actions={<PrintButton />}
          />
        </div>

        {/* Report body */}
        <article className="glass rounded-xl p-8 print:border-0 print:shadow-none space-y-8">
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              {RISK_CATEGORIES.map((r) => (
                <div key={r} className="rounded-xl border border-terminal-border p-3">
                  <div className="font-mono text-xl font-bold tabular-nums text-terminal-text">
                    {inventory.byRisk[r] ?? 0}
                  </div>
                  <div className="font-mono text-xs uppercase tracking-wider text-terminal-muted">
                    {r}
                  </div>
                </div>
              ))}
...
function Metric({ label, value, accent }: { label: string; value: string | number; accent?: boolean }) {
  return (
    <div className="rounded-xl border border-terminal-border p-3">
      <div
        className={`font-mono text-2xl font-bold tabular-nums ${accent ? 'text-terminal-amber' : 'text-terminal-text'}`}
      >
        {value}
      </div>
      <div className="mt-0.5 font-mono text-xs uppercase tracking-wider text-terminal-muted">
        {label}
      </div>
    </div>
  );
